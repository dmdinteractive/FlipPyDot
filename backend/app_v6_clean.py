...
cfg    = _load_json(os.path.join(BASE, "..", "config", "config.json"),
                    {"port": "/dev/cu.usbserial-BG01DCHX", "baud_rate": 57600})
...
def connect_serial():
    ...
    try:
        ...
        log.info(f"Connected: {PORT} @ {BAUD} — {W}x{H}")
        return True
...
def send_frame(frame):
    if not ser or not ser.is_open or panel is None:
        return False
    try:
...
def run_anim(fn, *args, **kwargs):
    ...
    def _r():
        ...
        for frame, delay in fn(*args, **kwargs):
            ...
            _emit_buf()
            time.sleep(delay)
    _anim_thread = threading.Thread(target=_r, daemon=True)
    _anim_thread.start()
...
def execute_cue(cue):
    ...
    if ct == "clear":
        ...
        y      = int(c.get("y", 0))
        if scroll:
            run_anim(anim_scroll_text, render_scrolling(txt, fname, fsize), W or 84)
        else:
            buffer = render_text(txt, fname, fsize, x, y)
            flush()
    elif ct == "animation":
        fn = get_animation(c.get("animation_id", "flash"))
        if fn:
            run_anim(fn, W or 84, H or 42, **c.get("params", {}))
    elif ct == "image":
        frames_data = c.get("frames", [])
        if frames_data:
            frames = [(np.array(f["bitmap"], dtype=np.uint8), f["duration"]) for f in frames_data]
            play_gif_frames(frames, c.get("loop", 1))
    _emit_status()
    log.info(f"Executed cue: {getattr(cue, 'number', '?')} [{ct}]")
...
def api_buf():
    return jsonify({"buffer": get_buf().tolist()})
...
def api_text():
    ...
    if scroll:
        ...
        hw  = H or 42
        ww  = W or 84
        buffer[:min(bmp.shape[0], hw), :min(bmp.shape[1], ww)] = \
            bmp[:min(bmp.shape[0], hw), :min(bmp.shape[1], ww)]
        flush()
    return jsonify({"success": True})
...
def api_image_display():
    ...
    if len(frames) == 1 and frames[0][1] == 0:
        ...
        play_gif_frames(frames, int(d.get("loop", 1)))
...
def api_vars_cfg():
    var_mod.configure(request.get_json() or {})
    return jsonify({"success": True})
...
@app.route("/api/cues")
def api_cues():
    return jsonify(cue_eng.get_status())
...
def api_upd_cue(cue_id):
    cue = cue_eng.update_cue(cue_id, request.get_json() or {})
    _emit_status()
    return jsonify({"success": bool(cue)})
...
def api_playlist():
    return jsonify(playlist.get_status())
...
def api_del_pl(item_id):
    playlist.remove(item_id)
    _emit_status()
    return jsonify({"success": True})

@app.route("/api/playlist/<item_id>", methods=["PUT"])
def api_upd_pl(item_id):
    playlist.update(item_id, request.get_json() or {})
    return jsonify({"success": True})

@app.route("/api/playlist/start", methods=["POST"])
def api_start_pl():
    d = request.get_json() or {}
    playlist.start(d.get("mode"))
    _emit_status()
    return jsonify({"success": True})
...
def api_move_pl():
    d = request.get_json() or {}
    playlist.move(d.get("id"), d.get("direction", "down"))
    return jsonify({"success": True})

# ── Assets ────────────────────────────────────────────────────────
@app.route("/api/assets")
def api_list_assets():
    return jsonify({"assets": asset_lib.list_all(
        request.args.get("type"), request.args.get("tag"))})

@app.route("/api/assets", methods=["POST"])
def api_create_asset():
    d = request.get_json() or {}
    a = asset_lib.create(d.get("name", "Untitled"), d.get("type", "text_preset"),
                         d.get("data", {}), d.get("tags", []))
    return jsonify({"success": True, "asset": a})

@app.route("/api/assets/search")
def api_search_assets():
    return jsonify({"assets": asset_lib.search(request.args.get("q", ""))})

@app.route("/api/assets/<asset_id>")
def api_get_asset(asset_id):
    a = asset_lib.get(asset_id)
    if a:
        return jsonify(a)
    return jsonify({"error": "Not found"}), 404

@app.route("/api/assets/<asset_id>", methods=["PUT"])
def api_upd_asset(asset_id):
    a = asset_lib.update(asset_id, request.get_json() or {})
    return jsonify({"success": bool(a)})

@app.route("/api/assets/<asset_id>", methods=["DELETE"])
def api_del_asset(asset_id):
    return jsonify({"success": asset_lib.delete(asset_id)})

# ── Effects ───────────────────────────────────────────────────────
@app.route("/api/effects")
def api_effects():
    return jsonify({**effects.get_status(), "registry": EFFECTS_REGISTRY})

@app.route("/api/effects", methods=["POST"])
def api_add_effect():
    d = request.get_json() or {}
    effects.add(d.get("name", "fx"), d.get("type", "flicker"), **d.get("params", {}))
    return jsonify({"success": True})

@app.route("/api/effects/<name>", methods=["DELETE"])
def api_del_effect(name):
    effects.remove(name)
    return jsonify({"success": True})
...
def api_load_show():
    d = request.get_json() or {}
    try:
        data = show_mgr.load_show(d.get("name", ""), cue_eng, scheduler)
        _emit_status()
